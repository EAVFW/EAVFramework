...
namespace DotNetDevOps.Extensions.EAVFramework.Hosting
{
    public class RoutingMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IEndpointRouter endpointRouter)
        {
            ...
            var eavEndpoint = endpoint.Metadata.GetMetadata<Endpoint>();

            var data = context.GetRouteData();
            await endpointRouter.ProcessAsync(context,context.RequestServices.GetService(eavEndpoint.Handler) as IEndpointHandler);
    ...
    public class EndpointRouter : IEndpointRouter
    {
        ...
        public EndpointRouter(IEventService events, ILogger<EndpointRouter> logger)
        {
            _events = events;
            _logger = logger;
        }
        public async Task ProcessAsync(HttpContext httpContext, IEndpointHandler endpoint)
        {
            try
            {
