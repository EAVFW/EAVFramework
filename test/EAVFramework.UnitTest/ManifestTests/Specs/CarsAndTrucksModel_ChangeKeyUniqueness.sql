...
IF NOT EXISTS(SELECT * FROM [manifest_migrations].[__MigrationsHistory] WHERE [MigrationId] = N'tests_1_0_0')
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_AccountingCode] ON [tests].[Cars] ([AccountingCode]) WHERE [AccountingCode] IS NOT NULL');
END;
GO
...
BEGIN
    DROP INDEX [IX_AccountingCode] ON [tests].[Cars];
END;
GO

IF NOT EXISTS(SELECT * FROM [manifest_migrations].[__MigrationsHistory] WHERE [MigrationId] = N'tests_1_0_1')
BEGIN
    CREATE INDEX [IX_AccountingCode] ON [tests].[Cars] ([AccountingCode]);
END;
GO
