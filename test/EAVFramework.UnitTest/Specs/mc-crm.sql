IF OBJECT_ID(N'[cmr].[__MigrationsHistory]') IS NULL
BEGIN
    IF SCHEMA_ID(N'cmr') IS NULL EXEC(N'CREATE SCHEMA [cmr];');
    CREATE TABLE [cmr].[__MigrationsHistory] (
...
BEGIN
    CREATE TABLE [tests].[Identities] (
        ...
        [AwesomeUserId] uniqueidentifier NOT NULL,
        ...
        CONSTRAINT [FK_Identities_Identities_AwesomeUserId] FOREIGN KEY ([AwesomeUserId]) REFERENCES [tests].[Identities] ([Id]),
...
IF NOT EXISTS(SELECT * FROM [cmr].[__MigrationsHistory] WHERE [MigrationId] = N'tests_1_0_1')
BEGIN
    INSERT INTO [cmr].[__MigrationsHistory] ([MigrationId], [ProductVersion])
